...
local function AddPlayerIdentitySection(tooltip)
	...
	if Addon.IsInLegionTimerunnerMode() then
		...
		local unbound =
			colorize(Addon.TooltipHelpers.GetLimitsUnboundRankString() .. " Limits Unbound", Colors.Artifact)
		local currentLine = tooltip:AddLine(threads .. " | " .. unbound)

		Addon.TooltipHelpers.AddHyperlinkTooltip(
			tooltip,
			currentLine,
			nil, -- whole line
			("spell:%d"):format(LIMITS_UNBOUND_SPELL_ID)
		)
	end
end
...
local function AddSubMenusSection(tooltip)
	...
	Addon.TooltipHelpers.AddSubTooltipLine(tooltip, "Infinite Research Quests", function(subTip)
