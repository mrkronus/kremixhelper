...
local SORT_FUNCS = {
	...
	totalThreads = function(a, b)
		return (a.totalThreads or 0) < (b.totalThreads or 0)
	end,
	limitsUnbound = function(a, b)
		return (a.limitsUnboundRank or 0) < (b.limitsUnboundRank or 0)
	end,
}
...
local function AddPartyRows(tooltip)
	...
	for _, m in ipairs(data) do
		...
		tooltip:SetLineScript(line, "OnMouseDown", function()
			...
			MenuUtil.CreateContextMenu(UIParent, function(_, root)
				...
				root:CreateTitle(headerText)

				-- Line 2: Threads | Limits (smaller font)
				local threadsText = Addon.FormatWithCommasToThousands(m.totalThreads or 0)
				...
				root:CreateButton(FOLLOW, function()
					FollowUnit(target)
				end)
			end, "cursor")
		end)
